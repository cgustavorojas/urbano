...
<form name='grid1' id='grid1' method='POST'>
	...
	<div id='abm-query-filtros'>
		...
		<tr><td colspan='<?php echo $screen->getFiltros()->getPosiciones(); ?>'><br>
			<input class='btn btn-primary btn-xs h btn-aceptar' type='submit' value='Exportar' onclick="$('grid1').action='export.php?cmd=exportar';">
			
			<?php if ($screenManager->getCount() > 1):?>
				<input class='btn btn-secondary btn-xs h btn-cancelar' type='submit' value='Volver' onclick="$('grid1').action='export.php?cmd=close';">
			<?php endif; ?>
		</td></tr>	
