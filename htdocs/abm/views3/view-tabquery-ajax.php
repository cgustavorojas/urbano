 ...
 */
	...
	if ($rs->getRowCount() > 0)
	{
			echo "<table class='table table-responsive table-striped table-view' cellspacing='2' cellpadding='0'>\n";
			...
			while ($row = $rs->getRow())
			{
				if (!is_null($screen->getRow())) {
					$row = array_merge ($screen->getRow(), $row);	// junto los datos de la cabecera con los ítems
				}
				...
				if (count($tab->getAccionesItem()) > 0)
				{
					...
					foreach ($tab->getAccionesItem() as $a)
					{
						if ($a->isEnabledForThisRow($row)) {
							...
							echo "  <a class='abm-action-item' href='#' onclick=\"doAction('$href', '$msgConfirm', $noMarkDouble); return false;\"><img src='$src' alt='$alt' title='$title'></img></a>\n";
						}
					}
					echo "</td>"; 
